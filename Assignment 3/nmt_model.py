...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(hidden_size, len(vocab.tgt), False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = torch.permute(self.model_embeddings.source(source_padded), (1, 2, 0))
        X = torch.permute(self.post_embed_cnn(X), (2, 0, 1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        embeddings = self.model_embeddings.target(target_padded)
        for Y_t in torch.split(embeddings, 1):
