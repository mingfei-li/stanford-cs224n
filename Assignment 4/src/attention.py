...
def precompute_rotary_emb(dim, max_positions):
    ...
    for i in range(dim//2):
        ...
        for t in range(max_positions):
            rope_cache[t, i, 0] = math.cos(theta_i * t)
            rope_cache[t, i, 1] = math.sin(theta_i * t)
...
def apply_rotary_emb(x, rope_cache):
    ...
    B, nh, T, dim = x.size()
    assert dim % 2 == 0
    x = torch.view_as_complex(x.view(B, nh, T, dim//2, 2))
    c = torch.view_as_complex(rope_cache[:T, :, :].view(1, 1, T, dim//2, 2))
    rotated_x = torch.view_as_real(x * c).view(B, nh, T, -1)
